...
<div class='px-6 mt-14 w-full lg:max-w-4xl pb-5'>
  <div class='w-full relative flex justify-around'>
    ...
    <div class='w-20 flex flex-col items-center z-10 shrink-0 {step > 1 ? 'text-foreground' : 'text-muted-foreground'}'>
      <Badge variant={step > 1 ? 'default' : 'secondary'} class='w-12 h-12 rounded-[50%] flex justify-center {step > 1 ? 'text-background' : 'text-muted-foreground'}' tabindex='-1'><Puzzle /></Badge>
      <div class='mt-3 font-bold'>
        Extensions
      </div>
    </div>
